...
-- A trigger is just like a flag, but takes a simple IO action to run
-- instead of a mutator for the options.  It can observe the options,
...
trigger ::
  ...
  -> String -- short description
  -> String -- extended description
  -> (o -> IO ()) -- the action to run
  -> OptSpec o
trigger _    snm lnm _    _        _
  | any ((=="-") . take 1) [snm,lnm] = dashesInOpt "trigger (flag)" snm lnm
trigger spec snm lnm desc ext_desc handler =
    OptSpec snm lnm "" desc ext_desc [OptAttrAllowUnset] noDefault (Just f) Nothing
  where f o = handler o >> return o
